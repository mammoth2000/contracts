...
interface IMammothReserve {
    //Mint backed tokens using collateral tokens
    function mint(
        uint256 collateralAmount
    ) external returns (uint256 backedAmount, uint256 feeAmount);

    //Estimate is a simple top level estimate that factors the processingFee
    function estimateMint(
        uint256 collateralAmount
    ) external view returns (uint256 backedAmount, uint256 feeAmount);
    ...
    function redeemCredit(
        ...
        returns (
            uint coreAmount,
            uint adjustedCoreAmount,
            uint coreAdjustedCreditAmount,
            uint feeAmount
        );

    //Only whitelisted
    function redeemCreditAsBacked(
        address destination,
        uint creditAmount
    ) external returns (uint backedAmount, uint feeAmount);

    //Estimates the redemption and uses collateralizationRatio to scale variable core component
    function estimateRedemption(
        uint256 backedAmount
    )
        external
        view
    ...
    function estimateCollateralToCore(
        uint collateralAmount
    ) external view returns (uint wethAmount, uint coreAmount);

    // This function is sensitive to slippage and that isn't a bad thing...
    // Estimates the amount of  core tokens getting transfered to USD collateral tokens
    function estimateCoreToCollateral(
        uint coreAmount
    ) external view returns (uint wethAmount, uint collateralAmount);
    ...
    function redeemCollateralCreditToWETH(
        uint256 collateralAmount
    ) external returns (uint wethAmount);
}
